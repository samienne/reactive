...
TEST(Stream, iterateSignalDifferentTypes)
{
    auto p = pipe<int>();
...
TEST(Stream, iterateSignalDifferentTypes)
{
    ...
    auto s1 = iterate(step, 2, pipe.output);

    EXPECT_EQ(2, s1.evaluate());

    pipe.input.push(true);
    signal::update(s1, signal::FrameInfo(1, us(0)));

    EXPECT_EQ(3, s1.evaluate());

    pipe.input.push(false);
    signal::update(s1, signal::FrameInfo(2, us(0)));
...
TEST(Stream, iterateCircular)
{
    ...
    auto s2 = signal::tee(iterate(add, i.signal, p.output), i.handle);
