...
namespace reactive::widget
{
    ...
    template <typename T>
    auto addWidgets(signal2::Signal<T, std::vector<Instance>> instances)
    {
        return makeInstanceModifier([](Instance instance, auto instances)
                {
                    return addWidgets(std::move(instance), std::move(instances));
    ...
    inline auto addWidget(signal2::AnySignal<Instance> instance)
    {
        std::vector<signal2::AnySignal<Instance>> instances;
