...
> **Status: implemented on `render-core` (PR #141).** R1-R5 have all landed
> and are CI-green. Supersedes an earlier `Session` model that bound the loop,
> one `RenderContext`, and its windows together; this note records why all three
> bindings dissolve and what replaced them. Reflects the branch as built.
>
...
An earlier `Session` model bound a run loop, one `RenderContext`,
...
covers all four without a per-backend reshape (keep this sequencing-level wording).
...
the earlier `Session` model). Upward-strong-refs are correct for steady-state destruction
...
- and the three reservations the earlier `Session` model had and this note dropped come back:
...
- **`REACTIVE_FRAMES=N` renders N frames on the dummy backend, then exits.** It
  posts a driver ahead of `run()` that pauses the loop and drives N deterministic
  `step(frameStep)` frames before stopping — the same pause/step path a manual
  driver uses, so it bounds static and animating scenes alike. It is a no-op under
  a remote driver: that client owns the clock and drives frames via `step` itself.
