...
namespace bqui::remote
{
    ...
    // macOS has no MSG_NOSIGNAL; suppress SIGPIPE per-socket instead. No-op
    // where the platform lacks the option (Linux uses kSendFlags/MSG_NOSIGNAL;
    // Windows has no SIGPIPE). Call on every connected fd.
    inline void suppressSigpipe(RawSocket sock)
    {
#ifdef SO_NOSIGPIPE
        int on = 1;
