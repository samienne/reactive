...
// First, so winsock2.h precedes the windows.h that pipetransport.h pulls in.
...
{
    // Reap a still-pending run-loop read so the kernel is done with our buffer
    // and OVERLAPPED before they are destroyed.
    if (readPending_ && handle_ != INVALID_HANDLE_VALUE)
    {
        CancelIoEx(handle_, &readyOverlapped_);
        DWORD got = 0;
        GetOverlappedResult(handle_, &readyOverlapped_, &got, TRUE);
    }
...
btl::NativeHandle PipeTransport::nativeHandle()
{
    // The run loop waits on the read-completion event; it never closes it.
    return btl::fromHandle(readEvent_);
}

bool PipeTransport::fillReady(std::string& out)
{
    for (;;)
    {
        if (!readPending_)
        {
            ResetEvent(readEvent_);
            readyOverlapped_ = OVERLAPPED{};
            readyOverlapped_.hEvent = readEvent_;

            DWORD got = 0;
            BOOL ok = ReadFile(handle_, readyChunk_, sizeof(readyChunk_), &got,
                    &readyOverlapped_);
            if (ok)
            {
                if (got == 0)
                ...
                out.append(readyChunk_, got);
                continue; // Drain whatever else is buffered.
            }
            ...
            {
                readPending_ = true;
                return true; // Armed: readEvent_ signals when data arrives.
            }
            if (error == ERROR_BROKEN_PIPE)
                return false;
        ...
        // A read is in flight; reap it without waiting.
        DWORD got = 0;
        if (GetOverlappedResult(handle_, &readyOverlapped_, &got, FALSE))
        {
            readPending_ = false;
            if (got == 0)
                return false;
            ...
            out.append(readyChunk_, got);
            continue; // Issue the next read.
        }

        DWORD error = GetLastError();
        if (error == ERROR_IO_INCOMPLETE)
            return true; // Still pending: nothing ready right now.
