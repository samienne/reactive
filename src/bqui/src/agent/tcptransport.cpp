...
btl::NativeHandle TcpTransport::nativeHandle()
{
#ifdef _WIN32
    return btl::fromSocket(static_cast<RawSocket>(sock_));
#else
    return btl::fromFd(static_cast<int>(static_cast<RawSocket>(sock_)));
#endif
}

bool TcpTransport::fillReady(std::string& out)
{
    auto sock = static_cast<RawSocket>(sock_);

#ifdef _WIN32
    // Windows recv() has no per-call non-blocking flag, so flip the socket to
    // non-blocking once. A transport is used either the blocking way or the
    // ready way, never both, so this does not disturb readAll().
    if (!nonBlocking_)
    {
        u_long on = 1;
    ...
    for (;;)
    {
...
#else
        ssize_t got = ::recv(sock, buffer, sizeof(buffer), MSG_DONTWAIT);
#endif
        if (got > 0)
        {
        ...
        if (err == WSAEWOULDBLOCK)
            return true; // Nothing more ready right now.
#else
        if (err == EINTR)
            continue;
        if (err == EAGAIN || err == EWOULDBLOCK)
            return true;
#endif
        return false; // Reset or hard error: disconnect.
    }
}
