...
namespace bqui::agent
{
    ...
    public:
        ...
        /**
         * @brief The OS handle to wait on for readability in a run loop.
         *
         * A socket fd/SOCKET, or the Windows pipe's overlapped-read event. The
         * base returns an invalid handle; the framed transports override it.
         * Non-owning: the transport still owns and closes the handle.
         */
        virtual btl::NativeHandle nativeHandle();

        /**
         * @brief Return every whole frame readable right now, without blocking.
         *
         * The run-loop alternative to receive(): on a readiness wake, drain
         * what is available and reassemble complete frames, buffering a partial
         * one for a later call. Returns empty when nothing whole is ready. When
         * the peer has gone, drains the last frames, then disconnected() is true.
         */
        virtual std::vector<std::string> receiveReady();

        /** @brief True once the peer has closed and receiveReady() has drained. */
