...
namespace bqui::remote
{
     ...
     *
     * The app launches **paused**; the client drives it. Each message is one
     * JSON-RPC 2.0 request, routed by `method` and answered with the client's
     * `id` — a `result`, or an `error` with a standard code (-32700 parse,
     * -32600 invalid request, -32601 method not found, -32602 invalid params,
     * -32603 internal). A message without an `id` is a notification and draws no
     * reply. `app.run` free-runs frames until `app.pause`; `app.step` advances a
     * bounded `count`; a query sees a whole, untorn frame either way. Returns
     * when the client sends `app.shutdown` or the channel closes. See
