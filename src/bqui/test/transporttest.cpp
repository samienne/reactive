...
// --- receiveReady (the non-blocking run-loop path) ------------------------

namespace
{
    // Poll receiveReady() until `count` frames arrive or we give up.
    std::vector<std::string> drainReady(Transport& t, size_t count)
    {
        std::vector<std::string> frames;
        for (int i = 0; i < 1000 && frames.size() < count; ++i)
        {
            for (auto& frame : t.receiveReady())
    ...
    // `reader` drains via receiveReady() while `writer` sends whole frames.
    void checkReceiveReadyFraming(std::unique_ptr<Transport> reader,
            std::unique_ptr<Transport> writer)
    {
        // A frame far larger than any OS buffer (so partial-frame buffering is
        // exercised), plus an empty and a small one back to back.
        std::string big(400 * 1024, 'x');
        for (size_t i = 0; i < big.size(); ++i)
            big[i] = static_cast<char>('a' + (i % 26));

        // Send on another thread so a large frame that fills the OS buffer does
        // not deadlock against the same-thread drain.
        std::thread sender([&]
        {
            writer->send("");
            writer->send("hi");
        ...
        auto frames = drainReady(*reader, 3);
        sender.join();

        ASSERT_EQ(3u, frames.size());
        EXPECT_EQ("", frames[0]);
        EXPECT_EQ("hi", frames[1]);
        EXPECT_EQ(big, frames[2]);
        EXPECT_FALSE(reader->disconnected());

        // The writer goes away; receiveReady() drains, then reports disconnect.
        writer.reset();
        bool gone = false;
        for (int i = 0; i < 1000 && !gone; ++i)
        {
            reader->receiveReady();
            gone = reader->disconnected();
            if (!gone)
                std::this_thread::sleep_for(std::chrono::milliseconds(2));
        }
        EXPECT_TRUE(gone);
...
TEST(transport, receiveReadyReassemblesFrames)
{
    // Default endpoint: a named pipe on Windows, a Unix socket on POSIX - so
    // this drives the pipe's overlapped state machine or the socket's
    // non-blocking recv, whichever the platform uses.
    auto endpoint = uniqueEndpoint("recvready");
    auto listener = listen(endpoint);
    ...
    checkReceiveReadyFraming(std::move(client), std::move(server));
}

TEST(tcpTransport, receiveReadyReassemblesFrames)
{
    auto listener = tcpListen(*parseTcpEndpoint("127.0.0.1:0"));
    auto endpoint = "127.0.0.1:" + std::to_string(listener->port());
