...
AnyWidget scrollView(AnyWidget widget)
{
    return makeWidget([](BuildParams params, auto widget)
    {
        ...
        auto viewSize = signal2::makeInput(avg::Vector2f(10.0f, 200.0f));
        auto x = signal2::makeInput(0.5f);
        auto y = signal2::makeInput(0.5f);
        ...
        auto dragOffset = signal2::makeInput(avg::Vector2f());
        auto scrollPos = signal2::makeInput<std::optional<avg::Vector2f>>(std::nullopt);
        ...
        auto view = bin(std::move(contentWidget), contentSize)
            | setSizeHint(signal2::constant(simpleSizeHint(
                {{100, 400, 10000}},
                {{100, 800, 10000}}
                )))
        ...
        auto makeBox = []()
        {
            return signal2::constant(simpleSizeHint(25.0f, 25.0f));
        };
